...
| # | Item | Status | Note |
|---|---|---|---|
| 5.1 | Service worker served a stale build | **PARTLY OBSERVED 2026-08-27 — cache sweep CONFIRMED, document auto-reload NOT** | The transition this row has waited for since 2026-08-17 finally had a real deploy to watch: a tab was opened at `sixonthedial.com`, confirmed **controlled** (which 5.1a records as the precondition a previous check destroyed), holding exactly one cache `wordy-800f543aa0ba`. 16 commits were pushed, the Pages deploy landed, and `sw.js` began serving `wordy-ede46bda935c`. **First correction, and it invalidates this row's own written test.** The instruction was "leave the tab open, let the deploy land, confirm the tab reloads itself". After 120 seconds idle the tab had NOT noticed: same cache, nothing installing, nothing waiting. That is not the fix failing — **browsers only check for a service-worker update on navigation** (or roughly every 24h). An idle tab never asks. The test as written cannot pass, and anyone running it would have concluded the fix was broken. **CONFIRMED, on `registration.update()` — exactly what the browser does on a navigation:** `controllerchange` fired; `caches.keys()` went from `[wordy-800f543aa0ba]` to **`[wordy-ede46bda935c]` — one entry, at the new stamp**, so the activate-time sweep deletes its predecessor rather than accumulating. The cached `/` is byte-identical to what the network serves. That is the substance of this row and it holds. **NOT CONFIRMED:** the document did not reload itself. `performance.now()` ran continuously across the transition and `navigation.type` stayed `navigate`, so no reload occurred — yet `hadController` was true, the component is mounted (`layout.tsx:224`), `controllerchange` is present in the shipped bundle, and `location.reload()` demonstrably works in that tab. None of the obvious explanations survives. The remaining candidate is that the update was forced from an isolated execution context, so the app's own listener may not have received the event the way it would on a real navigation. **Unresolved rather than passed or failed** — it needs one more observation on a natural navigation after a deploy. **Why this is not a release blocker even unresolved:** navigation is network-first with `cache: 'no-store'`, so a returning player fetches the current document regardless, and the cache is already swept. The auto-reload is belt-and-braces for a tab that happens to be open when a deploy lands, not the path by which anyone gets the new build. |
