...
| 3.2 | Google Play Developer — $25 once | **DONE 2026-08-27** | Organization account, `No Guesswork Systems LLC`, owner `toddswillis@gmail.com`, fee receipted. Organization rather than personal was the right call twice over: an individual account cannot be converted later without support, and the **12-testers-for-14-days closed-testing requirement applies only to personal accounts created after Nov 2023** — organizations are exempt, which saves two weeks at submission. **Verifications completed 2026-08-27** — the account is fully usable. One thing to reconcile: the phone field refused the LLC number as entered, so if a different line was verified, the entity block above and the `ngw-llc-phone` memory both still say (240) 782-0827 and one of them is now wrong. |
