...
(`build-puzzles.mjs 520`); the generator rejects candidates, so the ask and the
result are different numbers and only the second one is a fact. Why exactly two
fell out has not been chased.
...
- **`check:settings` and `check:intro`/`check:hydration` could not run here at
  all until this session.** Settings asserted a dark-mode colour without ever
  asking the browser for dark mode, so it failed on any machine not already in
  dark mode and passed on the author's; intro and hydration want the export
  served on `:4310` (`npx serve -l 4310 out`), the way CI does it. Neither was a
